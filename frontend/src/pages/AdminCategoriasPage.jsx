...
const AdminCategoriasPage = () => {
    ...
    const handleAdd = async (e) => {
        ...
        try {
            ...
            const endpoint = type === 'sector' ? '/api/catalogs/sectores' : '/api/catalogs/areas';
    ...
    const handleDelete = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const endpoint = type === 'sector' ? `/api/catalogs/sectores/${id}` : `/api/catalogs/areas/${id}`;
            const res = await fetch(endpoint, {
                method: 'DELETE',
                    ...
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-700 max-w-sm w-full transform transition-all animate-slide-up">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-xl font-extrabold text-slate-900 dark:text-white">
                                Añadir {addModal.type === 'sector' ? 'Sector' : 'Área'}
                            </h3>
                        ...
                        <form onSubmit={handleAdd}>
                            <div className="mb-6">
                                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">
                                    Nombre del {addModal.type === 'sector' ? 'sector' : 'área'}
                                </label>
                                <input
                                    type="text"
                                    autoFocus
                                    placeholder={addModal.type === 'sector' ? "Ej. Tecnología" : "Ej. Marketing"}
                                    className={`w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 outline-none focus:ring-2 transition-all font-medium text-slate-900 dark:text-white ${addModal.type === 'sector' ? 'focus:border-orange-500 focus:ring-orange-500/20' : 'focus:border-blue-500 focus:ring-blue-500/20'}`}
                                    value={newItemName}
                            ...
                            <div className="flex space-x-3">
                                ...
                                <button
                                    ...
                                    className={`flex-1 py-3 px-4 rounded-xl text-white font-bold active:scale-95 transition-all shadow-lg flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed ${addModal.type === 'sector' ? 'bg-orange-500 hover:bg-orange-600 shadow-orange-500/30' : 'bg-primary hover:bg-blue-600 shadow-primary/30'}`}
                                >
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fade-in">
                    ...
                    >
                        <div className="text-center">
                            ...
                            <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">¿Eliminar {deleteModal.type === 'sector' ? 'Sector' : 'Área'}?</h3>
